<div class="relative inline-block items text-left">
  <div>
    <button type="button" id="profile_btn" class="inline-flex items-center text-sm font-semibold leading-6 text-gray-900" aria-expanded="false">
      <span class="flex items-center text-xl font-semibold leading-6 text-gray-900 transition ease-in-out hover:text-[#4ba198]">
  ...
  <div id="profile_menu" class="hidden absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" tabindex="-1">
    <div class="py-1" role="none">
      <a href="{{route('pages.profile', [$id = Auth::user()->id])}}" class="text-gray-800 hover:text-[#4ba198] flex items-center gap-2 px-4 py-2 text-lg " tabindex="-1">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-6 h-6 hover:text-[#4ba198]">
      ...
      <form method="POST" action="{{route('auth.logout')}}" role="none">
        <button type="submit" class="flex gap-2 items-center text-gray-800 hover:text-[#4ba198] w-full px-4 py-2 text-left text-lg hover:underline" role="menuitem" tabindex="-1" id="menu-item-3">
